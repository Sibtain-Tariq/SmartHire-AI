...
export default function ScoreBreakdownCard({ title, score, explanation, icon: Icon, isLoading, isEmpty }) {
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
        <div className="flex items-center gap-2 text-slate-400">
          {Icon && <Icon size={18} />}
          <span className="font-semibold">{title}</span>
        </div>
        <div className="mt-4 flex flex-col items-center justify-center py-4 text-center">
          <span className="text-sm font-medium text-slate-500">Not enough data to calculate score.</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="group relative flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-slate-300 hover:shadow-md hover:-translate-y-0.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 font-semibold text-slate-900">
          {Icon && <Icon size={18} className="text-slate-400 group-hover:text-sky-500 transition-colors" />}
          {title}
        </div>
