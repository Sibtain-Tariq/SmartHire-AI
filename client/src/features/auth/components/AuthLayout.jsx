...
export default function AuthLayout({ children }) {
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="hidden w-1/2 flex-col justify-between border-r border-slate-200/80 bg-white p-12 lg:flex xl:w-5/12">
        {/* Logo */}
        <Link to="/" className="flex w-max items-center gap-3 rounded-xl outline-none transition focus-visible:ring-2 focus-visible:ring-sky-400">
          ...
          <span className="text-xl font-semibold tracking-tight text-slate-950">SmartHire AI</span>
        </Link>
        ...
        <div className="max-w-md">
          ...
          <motion.div 
            ...
            className="relative mt-12 flex h-48 w-full items-center justify-center overflow-hidden rounded-2xl border border-slate-200/60 bg-gradient-to-br from-slate-100 to-slate-50 shadow-sm"
          >
            {/* Abstract decorative elements */}
            <div className="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-sky-100 opacity-60 blur-2xl"></div>
            <div className="absolute -bottom-8 -left-8 h-32 w-32 rounded-full bg-indigo-100 opacity-60 blur-2xl"></div>
            <p className="relative z-10 text-sm font-medium text-slate-400">Illustration Placeholder</p>
          </motion.div>
        </div>
